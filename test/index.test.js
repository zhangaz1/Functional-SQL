Test.describe("SQL tests", function() {
	Test.it('chainify', function() {
		var q = query();
		Test.assertSimilar(q.select(), q);
		Test.assertSimilar(q.from(), q);
		Test.assertSimilar(q.where(), q);
		Test.assertSimilar(q.orderBy(), q);
		Test.assertSimilar(q.groupBy(), q);
		Test.assertSimilar(q.having(), q);
		Test.assertSimilar(q.execute(), []);
	});
	...
	Test.it('GROUP BY tests', function() {
		...
		Test.assertSimilar(query().select().from(persons).groupBy(profession, name, age, maritalStatus).execute(), [
			["teacher", [
				["Peter", [
					[20, [
						["married", [{
